...
#audio feature extractor files
VGGISH_PARAMS = 'https://storage.googleapis.com/audioset/vggish_model.ckpt'
VGGISH_PCA_PARAMS = 'https://storage.googleapis.com/audioset/vggish_pca_params.npz'
...
class YouTube8MFeatureExtractor(object):
  ...
  ** Note: OpenCV reverses the order of channels (i.e. orders channels as BGR
  ...
  def __init__(self, model_dir=MODEL_DIR, which_gpu=0):
    #set which gpu 
    ...
    # Download vggish PCA params
    self._maybe_download(VGGISH_PARAMS)
    # Download vggish params
    self._maybe_download(VGGISH_PCA_PARAMS)
  ...
  def _load_inception(self, proto_file):
    ...
    with self._inception_graph.as_default():
      ...
      #config = tf.ConfigProto(gpu_options=gpu_options)#, device_count = {'GPU': self.which_gpu}
